    ...
    for file_path in file_paths:
        try:
            ...
            for video_column in video_columns:
                ...
                # Generate x-values (frame numbers) for the smoothed data
                frame_numbers = np.arange(len(smoothed_data))
                
                # Plot the smoothed data
                plt.figure(figsize=(20, 6))
                plt.scatter(frame_numbers, smoothed_data, label=video_column, color='blue')
                plt.xticks(ticks=np.arange(0, len(frame_numbers), 5))
